...
public class Employee extends Person implements Payable, AttendanceTrackable, LeaveEligible, Comparable<Employee> {
    ...
    public Employee() {
        super();
        this.status = EmploymentStatus.PROBATIONARY;
    ...
    public boolean isActive() {
        return status != null && status != EmploymentStatus.CONTRACTUAL;
    }
    ...
    public boolean requiresTimeTracking() {
        return status == EmploymentStatus.REGULAR || status == EmploymentStatus.PROBATIONARY;
    }
    
    @Override
    public int getRequiredWorkingHoursPerDay() {
        return status == EmploymentStatus.PART_TIME ? 4 : 8;
    }
    
    @Override
    public boolean isEligibleForOvertime() {
        return status.isEligibleForOvertime();
    }
    
    // Implementing LeaveEligible interface (POLYMORPHISM)
    @Override
    public boolean isEligibleForLeave(String leaveType) {
        if (!status.isEligibleForBenefits()) return false;
    ...
    /**
     * Demonstrates POLYMORPHISM - shows different information based on actual class type
     * This method will show different results when called on Employee vs Manager vs Contractor
     */
    public String getEmployeeInfo() {
        return String.format("Employee: %s, Type: %s, Status: %s, Salary: %.2f", 
                            getDisplayName(), 
    ...
    // String status setter for backward compatibility
    public void setStatus(String status) {
        ...
        switch (statusUpper) {
            ...
            case "PROBATIONARY":
                ...
                break;
            case "CONTRACTUAL":
                this.status = EmploymentStatus.CONTRACTUAL;
                break;
            case "PART-TIME":
            case "PART_TIME":
                this.status = EmploymentStatus.PART_TIME;
                break;
            default:
                this.status = EmploymentStatus.PROBATIONARY;
        }
    ...
    // For compatibility with existing allowance system
    public double getRiceSubsidy() { return 0.0; } // Will be handled by allowance system
    public void setRiceSubsidy(double riceSubsidy) { /* Will be handled by allowance system */ }
    
    public double getPhoneAllowance() { return 0.0; } // Will be handled by allowance system
    public void setPhoneAllowance(double phoneAllowance) { /* Will be handled by allowance system */ }
    
    public double getClothingAllowance() { return 0.0; } // Will be handled by allowance system
    public void setClothingAllowance(double clothingAllowance) { /* Will be handled by allowance system */ }
    
    public double getTotalAllowances() { return calculateAllowances(); }
}